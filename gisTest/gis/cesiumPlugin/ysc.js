...
(function (window, undefined) {
    var ysc = {
        ...
        , creatEchartsRipple: creatEchartsRipple
        , initHaloLineMaterialProperty: initHaloLineMaterialProperty
        , creatHaloLine: creatHaloLine
    };

    /**
     * 绘制echarts散点图
     * @param data  数组类型，例如： [{name: '大庆', value: 279}];
     * @param geoCoordMap json类型，例如：{'大庆':[125.03,46.58]};
     * @param color 散点图颜色 ，默认'#f4e925'
     * @param shadowBlur 散点图波动大小，默认10
     * @param shadowColor 散点图波动颜色，默认'#333'
     */
    function creatEchartsRipple(data, geoCoordMap, color, shadowBlur, shadowColor) {
        var echartsColor = '#f4e925';
        var echartsShadowBlur = 10;
        var echartsShadowColor = '#333';
        if (color) {
            echartsColor = color;
        }
        if (shadowBlur) {
            echartsShadowBlur = shadowBlur;
        }
        if (shadowColor) {
            echartsShadowColor = shadowColor;
        }
            ...
            series: [
                {
                    ...
                    itemStyle: {
                        normal: {
                            color: echartsColor,
                            shadowBlur: echartsShadowBlur,
                            shadowColor: echartsShadowColor
                        }
                    },
                    zlevel: 1
                }
     ...
     * (流动)折线
     * */
    function creatHaloLine(viewer, data) {
        initHaloLineMaterialProperty(data);
        data.options.polyline.material = new Cesium.HaloLineMaterialProperty(data);
     ...
     * 光晕线材料
     */
    function initHaloLineMaterialProperty(data) {
        function HaloLineMaterialProperty(options) {
        ...
        Cesium.defineProperties(HaloLineMaterialProperty.prototype, {
            isConstant: {
                get: function () {
                    return false;
                }
        ...
        HaloLineMaterialProperty.prototype.getType = function (time) {
            return 'HaloLine';
        }
        HaloLineMaterialProperty.prototype.getValue = function (time, result) {
        ...
        HaloLineMaterialProperty.prototype.equals = function (other) {
            return this === other ||
                (other instanceof HaloLineMaterialProperty &&
                    Cesium.Property.equals(this._color, other._color))
        };
        Cesium.HaloLineMaterialProperty = HaloLineMaterialProperty;
        Cesium.Material.HaloLineType = 'HaloLine';
        Cesium.Material.HaloLineColor = data.color;
        Cesium.Material.HaloLineGlowPower = data.glowPower;
        Cesium.Material.HaloLineSource = " uniform vec4 color;\nuniform float glowPower;\n\nvarying float v_width;\n\n czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                                                       {\n\
                                                            ...
                                                            material.alpha = clamp(glow,0.0, 1.0 ) * color.a * abs(st.t - 0.5)*2.5;\n\
                                                            }\n\
                                                            return material;\n\
                                                        }";
        // material.HaloLineGlowPower:发光强度;material.HaloLineColor：颜色;
        Cesium.Material._materialCache.addMaterial(Cesium.Material.HaloLineType, {
            fabric: {
                type: Cesium.Material.HaloLineType,
                uniforms: {
                    glowPower: 0.1,
                    color: new Cesium.Color(1.0, 0.0, 0.0, 0.5)
                },
                source: Cesium.Material.HaloLineSource
            },
    ...
    //流动特效
    function initTrackMaterialProperty(data) {
        function TrackMaterialProperty(color, duration) {
            this._definitionChanged = new Cesium.Event();
            this._color = undefined;
            this._colorSubscription = undefined;
            this.color = color;
        ...
        Cesium.defineProperties(TrackMaterialProperty.prototype, {
            isConstant: {
                get: function () {
                    return false;
                }
        ...
        TrackMaterialProperty.prototype.getType = function (time) {
            return 'Track';
        }
        TrackMaterialProperty.prototype.getValue = function (time, result) {
            if (!Cesium.defined(result)) {
                result = {};
            }
            result.color = Cesium.Property.getValueOrClonedDefault(this._color, time, Cesium.Color.WHITE, result.color);
            result.image = Cesium.Material.TrackImage;
            result.time = (((new Date()).getTime() - this._time) % this.duration) / this.duration;
            return result;
        }
        TrackMaterialProperty.prototype.equals = function (other) {
            return this === other ||
                (other instanceof TrackMaterialProperty &&
                    Property.equals(this._color, other._color))
        };
        Cesium.TrackMaterialProperty = TrackMaterialProperty;
        Cesium.Material.TrackType = 'Track';
        Cesium.Material.TrackImage = data.flowImage;//图片
        Cesium.Material.TrackSource = "czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                                                       {\n\
                                                            czm_material material = czm_getDefaultMaterial(materialInput);\n\
                                                            vec2 st = materialInput.st;\n\
        ...
        Cesium.Material._materialCache.addMaterial(Cesium.Material.TrackType, {
            fabric: {
                type: Cesium.Material.TrackType,
                uniforms: {
                    color: new Cesium.Color(1.0, 0.0, 0.0, 0.5),
                    image: Cesium.Material.TrackImage,
                    time: 0
                },
                source: Cesium.Material.TrackSource
            },
            translucent: function (material) {
                return true;
            }
    ...
    function initPolylineTrailLinkMaterialProperty(data) {
        ...
        PolylineTrailLinkMaterialProperty.prototype.getValue = function (time, result) {
            ...
            result.glowPower = Cesium.Material.PolylineTrailLinkGlowPower;
            result.time = (((new Date()).getTime() - this._time) % this.duration) / this.duration;
            return result;
        }
        ...
        Cesium.Material.PolylineTrailLinkGlowPower = data.glowPower;//发光强度
        Cesium.Material.PolylineTrailLinkSource = "czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                                                       {\n\
                                                            czm_material material = czm_getDefaultMaterial(materialInput);\n\
                                                            vec2 st = materialInput.st;\n\
                                                            float glow = glowPower / abs(st.t - 0.5) - (glowPower / 0.5);\n\
                                                            vec4 colorImage = texture2D(image, vec2(fract(st.s*2.0 - time), st.t));\n\
                                                            material.emission = max(vec3(glow - 2.0 + (colorImage.rgb+color.rgb)/2.0), (colorImage.rgb+color.rgb)/2.0);\n\
                                                            material.alpha = colorImage.a * color.a;\n\
                                                            material.diffuse = (colorImage.rgb+color.rgb)/2.0;\n\
                                                            return material;\n\
                                                        }";
        ...
        Cesium.Material._materialCache.addMaterial(Cesium.Material.PolylineTrailLinkType, {
            fabric: {
                ...
                uniforms: {
                    ...
                    glowPower: Cesium.Material.PolylineTrailLinkGlowPower,
                    time: 0
                },
     ...
     * (流动)轨迹
     * */
    function creatTrackLine(viewer, data) {
        if (data.flowing == true) {
            initTrackMaterialProperty(data);
            var str1 = data.options.polyline.material[0];
            var str2 = data.options.polyline.material[1];
            data.options.polyline.material = new Cesium.TrackMaterialProperty(str1, str2);
        }
        viewer.entities.add(data.options);
    }
    ...
    function showDynamicLayer(viewer, data, callback) {
        ...
        function addLayer() {
            ...
            ysc.creatHtmlElement(viewer, element, divPosition, [-170, -(parseInt(element.css("height")) + 160)], true); //当为true的时候，表示当element在地球背面会自动隐藏。默认为false，置为false，不会这样。但至少减轻判断计算压力
        }
    }
    /**
